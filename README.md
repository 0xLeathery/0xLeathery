# Hey, I'm Ethan 👋

**Senior Consultant** at [Business Aspect](https://www.businessaspect.com.au/) (Data#3) · Brisbane, Australia

I'm a Microsoft ecosystem specialist who bridges the gap between enterprise architecture and hands-on development. I spent 3+ years at Microsoft as a Cloud Solution Architect and Customer Engineer focused on Dynamics 365 and Power Platform — and before that, I was building solutions across consulting firms from Auckland to Brisbane.

---

### What I Do

I work at the intersection of business process and technology — designing, building, and deploying Dynamics 365 and Power Platform solutions for government, education, and enterprise clients. My background spans solution architecture, customer engineering, technical consulting, and developer work, which means I can operate across the full lifecycle from discovery workshops through to production deployments.

On the development side, I build with **SvelteKit**, **React**, **Python**, **C#**, and **JavaScript** — from enterprise integrations to personal projects that scratch my own itch.

---

### Career

| Period | Role | Organisation |
|--------|------|-------------|
| 2025 – Present | **Senior Consultant** | Business Aspect (Data#3) |
| 2022 – 2025 | **Cloud Solution Architect & Customer Engineer** | Microsoft |
| 2021 | **Technical Consultant** — Dynamics & Power Platform | Clade Solutions |
| 2021 | **Technical Consultant** | Empired Ltd |
| 2020 – 2021 | **Associate Technical Consultant** | NTT Ltd |
| 2019 – 2020 | **Dynamics Developer** | JJ Richards & Sons |
| 2016 – 2019 | **Junior Software Developer** | JUCY Group (NZ) |
| 2015 | **Junior Network Engineer** | M2 Group (NZ) |

---

### Certifications

<details>
<summary><strong>Microsoft Certified Trainer (MCT)</strong> · Click to expand full certification list</summary>

<br>

**Dynamics 365**
- Dynamics 365 Sales Functional Consultant
- Dynamics 365 Marketing Functional Consultant  
- Dynamics 365 Field Service Functional Consultant
- Dynamics 365 Customer Engagement Functional Consultant

**Power Platform**
- PL-600 — Power Platform Solution Architect
- PL-400 — Power Platform Developer
- PL-200 — Power Platform Functional Consultant
- Power Platform App Maker

**Azure & M365**
- Azure fundamentals and enterprise implementations
- M365 deployment and Active Directory management

</details>

---

### Tech & Tools

**Enterprise** · Dynamics 365 (Sales, Marketing, Field Service, CE) · Power Platform (Power Apps, Power Automate, Power Pages) · Azure · M365 · Dataverse · Active Directory

**Development** · JavaScript · TypeScript · React · SvelteKit · Python · C# · .NET · Q#

**Other** · Solution Architecture · ALM & DevOps · Technical Training · Change Management

---

### What I'm Working On

- 🏗️ Enterprise architecture and modernisation projects across government and education
- 🛠️ Personal dev projects in SvelteKit and Python

---

### Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/ethanhurst/)
[![Email](https://img.shields.io/badge/Email-ethan.hurst@outlook.com.au-0078D4?style=flat&logo=microsoftoutlook&logoColor=white)](mailto:ethan.hurst@outlook.com.au)

<a href="https://gitroll.io/profile/uKV31ftMbI7gXWggXlJRCljqLKWu1" target="_blank"><img src="https://gitroll.io/api/badges/profiles/v1/uKV31ftMbI7gXWggXlJRCljqLKWu1?theme=light" alt="GitRoll Profile Badge"/></a>