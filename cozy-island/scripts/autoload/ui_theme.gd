...
func build_theme() -> Theme:
	...
	var panel := _make_textured_or_flat("res://assets/sprites/ui/panel_9slice.png", COLORS.bg_panel, 12)
	var panel_light := _make_textured_or_flat("res://assets/sprites/ui/panel_9slice.png", COLORS.bg_panel_light, 10)
	var button := _make_textured_button("res://assets/sprites/ui/button.png", COLORS.bg_panel_light, COLORS.accent)
	var button_hover := _make_textured_button("res://assets/sprites/ui/button_hover.png", COLORS.accent_soft, COLORS.accent, true)
...
func _make_textured_or_flat(texture_path: String, fallback: Color, radius: int) -> StyleBox:
	if ResourceLoader.exists(texture_path):
		...
		tex.content_margin_left = 14
		tex.content_margin_top = 12
		tex.content_margin_right = 14
		tex.content_margin_bottom = 12
		return tex
	return _make_panel_style(fallback, radius)


func _make_textured_button(texture_path: String, fallback_bg: Color, border: Color, bright: bool = false) -> StyleBox:
	if ResourceLoader.exists(texture_path):
		var tex := StyleBoxTexture.new()
		tex.texture = load(texture_path)
		tex.texture_margin_left = 8
		tex.texture_margin_top = 8
		tex.texture_margin_right = 8
		tex.texture_margin_bottom = 8
		tex.content_margin_left = 14
		tex.content_margin_top = 10
		tex.content_margin_right = 14
		tex.content_margin_bottom = 10
		if bright:
			tex.modulate_color = Color(1.08, 1.05, 1.0, 1.0)
		return tex
	return _make_button_style(fallback_bg, border, bright)
